...
BASE_FILE = DATA_DIR / "binary_routing_final.jsonl"
DRAFT_FILE = DATA_DIR / "clarify_candidates_draft.jsonl"
REVIEW_FILE = DATA_DIR / "clarify_candidates_review.md"
ACCEPTED_FILE = DATA_DIR / "clarify_candidates_accepted.jsonl"
MERGED_FILE = DATA_DIR / "binary_routing_expanded.jsonl"
SUMMARY_FILE = DATA_DIR / "binary_routing_expanded_summary.json"


def parse_review():
    text = REVIEW_FILE.read_text(encoding="utf-8")
    sections = re.split(r"(?=^## QHC\d{2}\s*$)", text, flags=re.MULTILINE)
    parsed = {}
    for section in sections:
        heading = re.match(r"^## (QHC\d{2})\s*(?:\n|$)", section)
        if not heading:
            continue
...
def main():
    ...
    if set(drafts) != set(reviewed):
        raise ValueError("Clarify review IDs do not match candidate draft IDs")
    ...
    if len(ids) != len(set(ids)):
        raise ValueError("Duplicate query IDs after merge")
    if len(queries) != len(set(queries)):
        raise ValueError("Duplicate user queries after merge")
    ...
    print(f"[Merged] {MERGED_FILE}", flush=True)
